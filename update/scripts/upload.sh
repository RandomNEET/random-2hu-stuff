...
LOCAL_PROJECT_ROOT="${PROJECT_ROOT:-$(cd "$SCRIPT_DIR/../.." && pwd)}"
FRONTEND_DIR="$LOCAL_PROJECT_ROOT/frontend"
BACKEND_DIR="$LOCAL_PROJECT_ROOT/backend"
REMOTE_TARGET="/var/www/"

# Required: loaded from .env or environment
: "${REMOTE_SERVER:?'REMOTE_SERVER is not set. Add it to .env'}"
: "${SSH_KEY:?'SSH_KEY is not set. Add it to .env'}"
: "${SSH_PORT:?'SSH_PORT is not set. Add it to .env'}"
...
check_dependencies() {
  ...
  local deps=("node" "npm" "rsync" "ssh" "ssh-agent" "ssh-add")
  for dep in "${deps[@]}"; do
...
test_ssh_connection() {
  ...
  if ssh -p "$SSH_PORT" -o ConnectTimeout=10 "$REMOTE_SERVER" "echo 'SSH connection successful'"; then
...
install_remote_dependencies() {
  ...
  if ssh -p "$SSH_PORT" "$REMOTE_SERVER" "cd $REMOTE_TARGET/random-2hu-stuff/backend && npm ci --production"; then
...
restart_pm2_on_remote() {
  ...
  if ssh -p "$SSH_PORT" "$REMOTE_SERVER" "cd $REMOTE_TARGET/random-2hu-stuff/backend && pm2 restart ecosystem.config.js --env production"; then
...
restart_remote_services() {
  ...
  # Restart PM2 service
  if ssh -p "$SSH_PORT" "$REMOTE_SERVER" "cd $REMOTE_TARGET/random-2hu-stuff/backend && pm2 restart ecosystem.config.js"; then
  ...
  if ssh -p "$SSH_PORT" "$REMOTE_SERVER" "systemctl reload nginx"; then
