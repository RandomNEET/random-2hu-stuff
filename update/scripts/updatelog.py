...
def get_dolt_commit(dolt_dir):
    """Get today's latest dolt commit hash from the dolt directory"""
    try:
...
def generate_update_entry(author_diff, video_diff, translated_diff, dolt_commit=None):
    ...
    dolt_url = (
        f"https://www.dolthub.com/repositories/{DOLTHUB_REPO}/compare/main/{dolt_commit}"
        if dolt_commit
        else f"https://www.dolthub.com/repositories/{DOLTHUB_REPO}"
    )
    ...
    html = f"""          <div class="update-item">
            ...
            <div class="update-content">
              <ul>
                ...
                <li>本次更新详情: <a href="{dolt_url}" target="_blank" rel="noopener noreferrer">dolthub</a></li>
              </ul>
            </div>
          </div>
...
def main():
    ...
    parser.add_argument(
        "--db-path",
        default=str(_project_root / "backend" / "random-2hu-stuff.db"),
