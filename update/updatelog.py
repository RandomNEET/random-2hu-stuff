...
def update_content_log(content_log_path, stats):
    ...
    try:
        ...
        with open(content_log_path, 'a') as f:
            ...
            f.write(f"{stats['totalAuthors']},{stats['totalVideos']},{stats['translatedVideos']}\n")
        print(f"Updated content.log with new statistics: {stats['totalAuthors']},{stats['totalVideos']},{stats['translatedVideos']}")
    except Exception as e:
...
def generate_update_entry(author_diff, video_diff, translated_diff):
    ...
    if author_diff > 0 and video_diff > 0 and translated_diff > 0:
        content_items.append(f"新收录作者{author_diff}位，视频{video_diff}个，熟肉{translated_diff}个")
    else:
...
def update_announcement_view(vue_file_path, update_entry):
    """Update the AnnouncementView.vue file with the new entry"""
    try:
        ...
        if match:
            ...
            print("Could not find insertion point in AnnouncementView.vue")
            return False
            
    except Exception as e:
        print(f"Error updating AnnouncementView.vue: {e}")
        return False
...
def main():
    ...
    vue_file_path = os.path.join(script_dir, '../frontend/src/views/AnnouncementView.vue')
    ...
    try:
        ...
        print(f"Current stats: Authors={current_stats['totalAuthors']}, Videos={current_stats['totalVideos']}, Translated={current_stats['translatedVideos']}")
        ...
        print(f"Last stats: Authors={last_stats['totalAuthors']}, Videos={last_stats['totalVideos']}, Translated={last_stats['translatedVideos']}")
        ...
        translated_diff = current_stats['translatedVideos'] - last_stats['translatedVideos']
        
        print(f"Differences: Authors=+{author_diff}, Videos=+{video_diff}, Translated=+{translated_diff}")
        ...
        if update_announcement_view(vue_file_path, update_entry):
