...
app.get("/api/search/videos", (req, res) => {
  ...
  refreshSearchCache((err) => {
    if (err) {
      return res
        .status(500)
        .json({ error: "Failed to refresh search cache: " + err.message });
    }
    ...
    if (filters.author !== "all") {
      console.log(
        ...
        filters.author,
        "type:",
        typeof filters.author,
      );
      filteredVideos = filteredVideos.filter(
        (v) => v.author === filters.author,
      );
    ...
    if (filters.dateFrom) {
      filteredVideos = filteredVideos.filter(
        (v) => v.date && v.date >= filters.dateFrom,
      );
    }
    if (filters.dateTo) {
      filteredVideos = filteredVideos.filter(
        (v) => v.date && v.date <= filters.dateTo,
      );
    }
    ...
    const results = searchResults.slice(0, filters.limit).map((result) => {
    ...
    const finalResults = results.map((video) => ({
      ...
      translation_status_text:
        TRANSLATION_STATUS[video.translation_status] || "未知",
...
app.get("/api/search/authors", (req, res) => {
  ...
  refreshSearchCache((err) => {
    ...
    const results = searchResults.slice(0, limit).map((result) => ({
      ...
      displayName:
        result.item.yt_name ||
        result.item.nico_name ||
        result.item.twitter_name ||
        "Unknown",
    }));
